...
# PROTECTED & PRIVATE ####################################################
    ...
    private function __end_project($number, $endDate)
    {
        $sql_params = array(
            ":" . ProjectDBObject::COL_NUMBER => $number,
            ":" . ProjectDBObject::COL_STATUS => Project::STATUS[3],
    ...
    private function __bad_end_project($number, $endDate, $content, $attributable)
    {
        ...
        $sql_params = array(
            ":" . ProjectDBObject::COL_NUMBER => $number,
            ":" . ProjectDBObject::COL_STATUS => Project::STATUS[5],
            ":" . ProjectDBObject::COL_END_DATE => $endDate,
            ":" . ProjectDBObject::COL_UPDATE_DATE => date('Y-m-d H:i:s')
        );
        // create sql request
    ...
    private function __archive_project($number)
    {
        ...
        if ($project->getStatus() != Project::STATUS[3] && $project->getStatus() != Project::STATUS[5]) {
            return false;
        }
        $sql_params = array(
            ":" . ProjectDBObject::COL_NUMBER => $number,
...
class ProjectDBObject extends AbstractDBObject
{
    ...
    public function __construct($module)
    {
        ...
        $dol_amendment
            ...
            ->AddForeignKey(ProjectDBObject::TABL_AMENDMENT . '_fk1', ProjectDBObject::COL_TYPE, ProjectDBObject::TABL_AMENDMENT_TYPE, ProjectDBObject::COL_LABEL, DBTable::DT_RESTRICT, DBTable::DT_CASCADE)
            ->AddForeignKey(ProjectDBObject::TABL_AMENDMENT . '_fk2', ProjectDBObject::COL_PROJECT_NUMBER, ProjectDBObject::TABL_PROJECT, ProjectDBObject::COL_NUMBER, DBTable::DT_CASCADE, DBTable::DT_CASCADE);
